...
const RELEASE_URL: &str = "https://github.com/bao1237885/openclaw/releases/latest";
#[cfg(any(target_os = "macos", target_os = "windows"))]
// Test desktop builds need a channel that Linux-only releases never replace.
const DESKTOP_TEST_UPDATE_ENDPOINT: &str =
    "https://updates.starship.invalid/latest-desktop.json";
    ...
    let update = match updater.check().await {
        ...
        Ok(None) => {
            deliver_result(
                ...
                NOT_AVAILABLE_EVENT,
                (),
                "Starship is up to date — no update is available",
            );
            return;
        }
        Err(error) => {
