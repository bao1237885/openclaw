...
const slackSetupAdapterBase = createPatchedAccountSetupAdapter({
  ...
  validateInput: ({ cfg, accountId, input }) => {
    ...
    const identity = setupInput.identity ?? account.config.identity ?? "bot";
  ...
  buildPatch: (input) => {
    const setupInput = input as SlackSetupInput;
    return {
      ...(setupInput.identity ? { identity: setupInput.identity } : {}),
...
export const slackSetupAdapter: ChannelSetupAdapter = {
  ...
  applyAccountConfig: ({ cfg, accountId, input }) => {
    const setupInput = input as SlackSetupInput;
    const identity = setupInput.identity ?? inspectSlackAccount({ cfg, accountId }).config.identity;
