...
2. 壳层原生能力（浏览器面板、本地应用发现等）**不再从 8.1 移植**，全部在 9.3 壳层重新实现。
...
| **B 壳层轴** | Tauri 桌面壳（窗口、托盘、网关守护、原生 WebView2 浏览器面板）       | 星舰 `2.0.0`（基于官方 9.3 壳） | 我们自己             | 外壳、启动页、原生面板                   |

**关键结论：壳层版本不决定用户看到的 UI。**
用户看到的对话/任务界面来自 A（引擎侧官方 Control UI）。所以：

- 壳层直接用官方 9.3 基线 → 只是换「外壳」，UI 仍是官方 9.3 Control UI；
- 壳层原生能力按 9.3 的接口从零实现，**不回退到 8.1 源码**；
- 8.1 那套「自研 UI 顶替官方 + `starship-zh-inject.js` DOM 注入」才是反复崩溃的根源，**不移植**。
...
| `starship-shell/v1.0.0-8.1` (tag)      | 历史归档：8.1 壳（指向 `9d39389c0a`），只读参考，不作为基线 |
| `openclaw-upstream-archive/<version>/` | 每次正式版上游整仓备份                                      |

> 分支名固定为 `starship/product-9.3`；`product-8.1` 及 8.1 壳源码一律视为历史归档，不再作为开发基线。

### 5.2 壳源码防丢（P0）

当前壳源码在 `apps/starship-desktop`（官方 9.3 壳 + 品牌化）。需要保证：

1. 8.1 壳归档 tag `starship-shell/v1.0.0-8.1` 保留（只读参考）；
...
- 我们自己新增的原生模块（如 `native_browser.rs`）放在独立文件，**不改官方文件的行**，所以合并时几乎不会冲突。
...
| 0.6 | 确定壳层原生能力的**实现位置与接口**（不写业务代码）                    | 见下方清单                                                                                                                                                                |

**0.6 原生能力清单（全部在 9.3 壳层从零实现，禁止移植 8.1 源码）**

| 能力                                                       | 实现位置（9.3 壳层）                                            | 依赖的官方接口                                                          |
| ---------------------------------------------------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------- |
| 原生 WebView2 浏览器面板                                   | `apps/starship-desktop/src-tauri/src/native_browser.rs`（新增） | `ui/src/app/native-browser-bridge.ts` 定义的 `openclawBrowser` 消息协议 |
| 本地应用发现/启动                                          | 后续独立文件（按需新增）                                        | Tauri shell / Windows API，按 9.3 壳层现有模块风格实现                  |
| Windows 进程树回收                                         | 后续独立文件（按需新增）                                        | Win32 Job Object，按需实现                                              |
| `gateway*.rs` / `tray.rs` / `updater.rs` / `quickchat*.rs` | **不实现，直接用官方 9.3 版本**                                 | 官方已有且更新                                                          |
| `starship-zh-inject.js` / 自研 UI 顶替                     | **禁止**                                                        | 已列入废弃清单                                                          |
...
| 1.2 | 原生浏览器面板改为壳层 WebView2 子视图（9.3 壳层新写 `native_browser.rs`），**不注入官方 DOM** |
...
**历史根因（已定位）**：1.0.0 用「壳层自带 UI 产物 + 打补丁」的方式接管浏览器面板，导致官方 UI 升级即覆盖、面板跳窗/白屏。修法：**官方 UI 保真 + 壳层原生 WebView2 实现 `native-browser-bridge` 协议**。
...
| #   | 动作                                                       | 位置                                                    |
| --- | ---------------------------------------------------------- | ------------------------------------------------------- |
| 2.1 | `native_browser_dispatch` 通用 CDP 命令                    | `apps/starship-desktop/src-tauri/src/native_browser.rs` |
| 2.2 | `native_browser_act` 统一入口（elementRef / x,y / scroll） | 同上                                                    |
| 2.3 | `act()` / `dispatch()`                                     | 壳层桥注入脚本                                          |
| 2.4 | 前端面板接 `computer.act` 语义                             | 官方面板（无需改官方源码）                              |
...
| `product-8.1` 当唯一权威壳            | 基线过旧，带着旧 UI 包袱                                 | `starship/product-9.3` 壳基线 |
...
| 8.1 壳历史归档                       | `backup/xb-starship-product-8-1` @ `9d39389c0a`（只读参考，当前基线是官方 9.3 壳）                                                  |
